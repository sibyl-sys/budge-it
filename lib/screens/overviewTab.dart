...
class OverviewTab extends StatefulWidget {
  ...
  const OverviewTab({Key? key, required this.from, required this.to})
      : super(key: key);
...
class _OverviewTabState extends State<OverviewTab>
  ...
  BarTooltipItem getTooltipItem(group, groupIndex, rod, rodIndex) {
    ...
    if (rangeType == DateRangeType.MONTHLY) {
      ...
      text = toolTipFormat.format(
              DateTime(widget.from.year, widget.from.month, group.x.toInt())) +
          ": " +
          rod.toY.toString();
    }
  ...
  Widget bottomTitles(double value, TitleMeta meta) {
    ...
    return SideTitleWidget(
        child: Text(text,
            style: TextStyle(color: Color(0xFFB6B6B6), fontSize: 10)),
  ...
  List<BarChartGroupData> getData(
    ...
    for (int i = 0; i <= barCount; i++) {
      ...
      if (getSelectedType() == 0) {
        categories.forEach((category) {
          ...
          if (rangeType == DateRangeType.MONTHLY) {
            ...
            categoryValue = user.getCategoryNet(
                from: DateTime(from.year, from.month, i + from.day),
                to: DateTime(from.year, from.month, from.day + i),
                categoryID: category.categoryID);
          }
          total += categoryValue;
          rodValues.add({"value": categoryValue, "color": category.color});
          //ROD DATA
        });
        ...
        TransactionImportance.values.forEach((importance) {
          ...
          rodValues.add({"value": categoryValue, "color": importanceColor});
        });
      } else {
        double netValue = 0;
        if (rangeType == DateRangeType.MONTHLY) {
      ...
      rodValues.forEach((e) {
        if (e["value"] != 0) {
          print(e["value"]);
          rodData.add(BarChartRodStackItem(lastValue, lastValue + e["value"],
              Color(e["color"]).withOpacity(1)));
          lastValue += e["value"];
        }
      });
      ...
      children: [
        ...
        Container(
          ...
          child: ToggleButtons(
            ...
            constraints:
                BoxConstraints(minHeight: 35, minWidth: 110, maxWidth: 120),
            ...
            ? ColoredBox(
                ...
                child: TabBar(
                  ...
                  unselectedLabelColor:
                      _tabController.index == 1 ? Colors.red : Colors.teal,
                  tabs: [
                    Tab(
                      child: Column(
                          ...
                          RichText(
                            text: TextSpan(
                                ...
                                children: [
                                  TextSpan(
                                      text:
                                          "${moneyFormat.format(user.getCategoryTypeNet(from: widget.from, to: widget.to, categoryType: CategoryType.expense).abs()).split('.')[0]}",
                                      style: TextStyle(
                                          color: Color(0xffEB6467),
                                          fontSize: 14,
                                          fontFamily: "Poppins",
                                  ...
                                  TextSpan(
                                      text:
                                          ".${moneyFormat.format(user.getCategoryTypeNet(from: widget.from, to: widget.to, categoryType: CategoryType.expense).abs()).split('.')[1]}",
                                      style: TextStyle(
                                          color: Color(0xffEB6467),
                                          fontSize: 12,
                                          fontFamily: "Poppins",
                          ...
                          RichText(
                            text: TextSpan(
                                ...
                                children: [
                                  TextSpan(
                                      text:
                                          "${moneyFormat.format(user.getCategoryTypeNet(from: widget.from, to: widget.to, categoryType: CategoryType.income).abs()).split('.')[0]}",
                                      style: TextStyle(
                                          color: Color(0xff55C9C6),
                                          fontSize: 14,
                                          fontFamily: "Poppins",
                                  ...
                                  TextSpan(
                                      text:
                                          ".${moneyFormat.format(user.getCategoryTypeNet(from: widget.from, to: widget.to, categoryType: CategoryType.income).abs()).split('.')[1]}",
                                      style: TextStyle(
                                          color: Color(0xff55C9C6),
                                          fontSize: 12,
                                          fontFamily: "Poppins",
